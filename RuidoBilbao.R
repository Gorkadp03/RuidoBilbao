...
sonometro_sf <- sonometro_mediciones_df %>%
...
sonometro_sf <- sonometro_sf[!is.na(st_dimension(sonometro_sf)), ]
...
# Usamos 'sonometro_sf' que es tu variable correcta
idx <- st_nearest_feature(sonometro_sf, trafico_tramos_unicos)

# Asignamos el ID de la calle (CodigoSeccion) al sonómetro
sonometro_sf$CodigoSeccion <- trafico_tramos_unicos$CodigoSeccion[idx]

# --- PASO 2: Sincronización Temporal (Redondeo a 15 min) ---
# Redondeamos la hora del SONÓMETRO
sonometro_sf <- sonometro_sf %>%
